						...
						$(list)
								.each(
										function(i) {
											...
											newDate.setTime(this.cn_activity_end_time);
											var column = i % 3;
											$('#col_' + column)
													.append(
															'<div id="contentfeeds'
																	+ i
																	+ '" class="panel panel-animated panel-default animated fadeInUp" style="visibility: visible;"><div class="panel-body bordered-bottom"><div class="no-padding jumbotron '
																	+ color
																	+ '"><p class="lead"><a href="activity_detail.html#'
																	+ this.cn_activity_id
																	+ '">'
																	+ this.cn_activity_title
																	+ '</a></p></div><p class="text-muted">'
																	+ this.cn_activity_body
																	+ '</p><div class="text-muted"><small style="color:red;">活动结束时间:'
																	+ newDate.format('yyyy年MM月dd h:m:s')
																	+ '</small></div></div></div>');
										});
					} else {
		...
		success : function(result) {
			if (result.status == 0) {
				...
				$("#content_body")
						.append(
								'<h4><strong>标题: </strong>'
										+ noteActivity.cn_note_activity_title
										+ '</h4>');
						...
						$(list)
								.each(
										function() {
											$("#select_notebook ul")
													.append(
															'<li class="online"><a ><i class="fa fa-book" title="online" rel="tooltip-bottom"></i> '
																	+ this.cn_notebook_name
																	+ '</a></li>');
											$('#select_notebook li:last').data(
													'notebook', this);
										});
					} else {
						alert(result.message);
						...
						$("#first_action ul")
								.prepend(
										'<li class="online"><a ><i class="fa fa-file-text-o" title="online" rel="tooltip-bottom"></i> '
												+ noteActivity.cn_note_activity_title
												+ '<button type="button" class="btn btn-default btn-xs btn_position_3 btn_up"><i class="fa fa-thumbs-o-up"></i></button><button type="button" class="btn btn-default btn-xs btn_position_2 btn_down"><i class="fa fa-thumbs-o-down"></i></button><button type="button" class="btn btn-default btn-xs btn_position btn_like"><i class="fa fa-star-o"></i></button></a></li>');
						$("#first_action ul li:first").data("noteActivity",
								noteActivity);
